...
const SelectScreen = (() => {
  ...
  function render() {
    ...
    if (session) {
      ...
      // Conquista escolhida pelo usuário (só uma por vez)
      try {
        if (window.Achievements) {
          const displayed = Achievements.getDisplayed();
          if (displayed) {
            const badge = Achievements.createBadge(displayed);
      ...
      // ── Botão Mesas ──
      const mesasBtn = document.createElement('button');
      mesasBtn.className = 'btn-ghost';
      mesasBtn.textContent = '🎲 Mesas';
      mesasBtn.addEventListener('click', function() {
        Dice.sounds.page();
        App.go('mesas');
      });
      right.appendChild(mesasBtn);

      // ── Botão Painel do Adm (só aparece para administradores) ──
      if (window.Account && Account.isAdmin(user)) {
        const adminBtn = document.createElement('button');
        adminBtn.className = 'btn-ghost';
        adminBtn.textContent = '👑 Painel do Adm';
        adminBtn.style.borderColor = 'var(--gold-dim)';
      ...
      if (chars.length) {
        ...
        chars.forEach(function(char) {
          ...
          delBtn.addEventListener('click', function(e) {
            e.stopPropagation();
            Modal.confirm('Tem certeza que deseja excluir "' + (char.name || 'Sem nome') + '"?', {
              title: '🗑️ Excluir personagem',
              icon: '🗑️',
              confirmText: 'Excluir'
            }).then(function(ok) {
              if (!ok) return;
