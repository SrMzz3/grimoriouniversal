...
const TableUI = (() => {
  ...
  function renderTableManager() {
    ...
    createBtn.addEventListener('click', function() {
      Modal.prompt('Nome da mesa:', { title: '✦ Nova Mesa', icon: '🎲', placeholder: 'Ex: A Ordem de Vigília' })
        .then(function(name) {
          if (!name) return;
          return TableSystem.createTable(name)
            .then(function(table) {
              try { Achievements.track('table_created', { id: table.id }); } catch (e) {}
              App.go('mesas');
            });
        })
        .catch(function(err) { Toast.error(err.message || err, 'Erro ao criar mesa'); });
    });
      ...
      .then(function(tables) {
        ...
        tables.forEach(function(table) {
          ...
          leaveBtn.addEventListener('click', function() {
            Modal.confirm('Sair da mesa?', { title: '🚪 Sair', icon: '🚪', confirmText: 'Sair' })
              .then(function(ok) {
                if (!ok) return;
                return TableSystem.leaveTable(table.id)
                  .then(function() { App.go('mesas'); });
              })
  ...
  function checkTableLink() {
    ...
    if (tableId) {
      TableSystem.joinTable(tableId)
        ...
        .catch(function(err) {
          Toast.error(err.message || err, 'Erro ao entrar na mesa');
        });
      return true;
    }
    return false;
    ...
    chars.forEach(char => {
      ...
      if (char.canViewFull || char.isOwner) {
        ...
        viewBtn.addEventListener('click', function() {
          ...
          Modal.alert(
            `${char.cls || 'Sem classe'} · Nível ${char.level || 1}<br>❤️ ${char.hpCur}/${char.hpMax}`,
            { title: char.name, icon: '👤' }
          );
        });
      }
  ...
  // ── Painel do Mestre ──
  function renderMasterPanel(tableId, characters) {
    const panel = document.createElement('div');
    panel.className = 'card';
    panel.style.marginTop = '16px';
    panel.style.background = 'rgba(16,15,12,0.85)';
    ...
    title.className = 'card-title';
    title.textContent = '👑 Painel do Mestre';
    panel.appendChild(title);

    const gameChartSection = document.createElement('div');
    gameChartSection.id = 'game-chart-section';
    panel.appendChild(gameChartSection);

    const radarSection = document.createElement('div');
    radarSection.style.marginTop = '16px';
    radarSection.style.paddingTop = '16px';
    radarSection.style.borderTop = '1px solid var(--border)';
    radarSection.id = 'radar-section';
    panel.appendChild(radarSection);

    const barsSection = document.createElement('div');
    barsSection.style.marginTop = '16px';
    barsSection.style.paddingTop = '16px';
    barsSection.style.borderTop = '1px solid var(--border)';
    barsSection.id = 'bars-section';
    panel.appendChild(barsSection);

    return panel;
  }

  function updateTableCharts(tableId) {
    const user = StorageAdapter.getCurrentUser();
    if (!user) return;

    TableSystem.isMaster(tableId, user.id)
      .then(isMaster => {
        if (!isMaster) return;
        return TableSystem.getTableCharacters(tableId, user.id);
      })
      .then(characters => {
        if (!characters || characters.length === 0) {
          const section = document.querySelector('#game-chart-section');
          if (section) {
            section.innerHTML = '<p style="color:var(--ink-faint);text-align:center;padding:10px;">Nenhum personagem compartilhado.</p>';
          }
          return;
        }

        const gameSection = document.getElementById('game-chart-section');
        const radarSection = document.getElementById('radar-section');
        const barsSection = document.getElementById('bars-section');

        if (gameSection) {
          gameSection.innerHTML = '';
          Charts.renderGameChart(characters, gameSection);
        }
        ...
        if (radarSection) {
          ...
          radarTitle.style.cssText = `
            ...
            text-align: center;
          `;
          radarTitle.textContent = '📊 Comparação de Atributos';
          radarSection.appendChild(radarTitle);
          Charts.renderRadar(characters, radarSection);
        }

        if (barsSection) {
          barsSection.innerHTML = '';
          Charts.renderStrengthBars(characters, barsSection);
        }
      })
  ...
  function renderTableScreen(tableId) {
    ...
    shareBtn.addEventListener('click', function() {
      ...
      if (!char) {
        Toast.warning('Nenhum personagem carregado!');
        return;
      }
    ...
    TableSystem.isMaster(tableId, user.id)
      .then(isMaster => {
        if (isMaster) {
          const masterPanel = renderMasterPanel(tableId, []);
          wrap.appendChild(masterPanel);
          updateTableCharts(tableId);
        }
      })
      .catch(() => {});
    ...
    function sendChatMessage() {
      ...
      TableSystem.sendMessage(tableId, msg)
        .then(() => {
          input.value = '';
          try { if (window.Achievements) Achievements.track('message_sent', { tableId: tableId }); } catch (e) {}
          loadChat();
        })
        .catch(err => Toast.error(err.message || err));
    }
