...
const ProfileScreen = (() => {
  ...
  // Modo edição vive só em memória: sair da tela sempre volta pra visualização
  let editing = false;

  // Chamado pelo App: sempre abre em modo VISUALIZAÇÃO
  function render() {
    editing = false;
    return renderView();
  }

  // Re-render interno (mantém o modo atual)
  function rerender() {
  ...
  function renderView() {
    ...
    toggleBtn.addEventListener('click', function() {
      if (isEditing) {
        // Salva o que estiver nos campos antes de voltar pra visualização
        const nameField = document.getElementById('edit-displayname');
        const bioField = document.getElementById('edit-bio');
        if (nameField) user.displayName = nameField.value.trim() || user.username;
        if (bioField) user.bio = bioField.value;
        GrimorioStorage.updateCurrentUser(user);
        if (window.StorageAdapter && StorageAdapter.updateUser) {
          StorageAdapter.updateUser(user).catch(function() {});
        }
        editing = false;
        Dice.sounds.magic();
    ...
    // ── Badge da conquista escolhida (só no modo visualização) ──
    if (!isEditing && window.Achievements) {
      try {
        const displayedId = Achievements.getDisplayed();
        if (displayedId) {
          const badge = Achievements.createBadge(displayedId);
          if (badge) {
            const badgeRow = document.createElement('div');
            badgeRow.style.marginTop = '10px';
            badgeRow.appendChild(badge);
            const nameEl = userInfo.firstElementChild;
            if (nameEl && nameEl.nextSibling) {
              userInfo.insertBefore(badgeRow, nameEl.nextSibling);
            } else {
              userInfo.appendChild(badgeRow);
            }
          }
        }
      } catch (e) {}
    ...
    if (isEditing) {
      ...
      FRAMES.forEach(f => {
        ...
        btn.addEventListener('click', function() {
          const changed = user.frame !== f.id;
          user.frame = f.id;
          GrimorioStorage.updateCurrentUser(user);
          if (changed) {
            try { if (window.Achievements) Achievements.track('frame_changed', { frame: f.id }); } catch (e) {}
          }
          applyFrame(f.id);
      ...
      saveRecBtn.addEventListener('click', function() {
        ...
        if (keyword.length < 3) {
          Toast.warning('A palavra-chave deve ter pelo menos 3 caracteres.');
          return;
        }
      ...
      deleteBtn.addEventListener('click', function() {
        Modal.confirm('Tem certeza que deseja EXCLUIR PERMANENTEMENTE sua conta e todos os dados? Essa ação é IRREVERSÍVEL.', {
          title: '⚠️ Zona de Perigo',
          icon: '🗑️',
          confirmText: 'Continuar'
        }).then(function(ok) {
          if (!ok) return;
          return Modal.prompt('Digite seu nome de usuário para confirmar:', {
            title: '🗑️ Confirmar exclusão',
            icon: '⚠️',
            placeholder: user.username
          }).then(function(userInput) {
            if (userInput !== user.username) {
              if (userInput !== null) Toast.error('Nome incorreto. Exclusão cancelada.');
              return;
            }
            const finish = function() {
              const users = JSON.parse(localStorage.getItem('grimorio_users') || '[]');
              localStorage.setItem('grimorio_users', JSON.stringify(users.filter(function(u) { return u.id !== user.id; })));
              localStorage.removeItem('grimorio_session');
              const bg = document.querySelector('.profile-bg');
              if (bg) bg.remove();
              location.reload();
            };
            if (window.StorageAdapter && StorageAdapter.deleteUser) {
              return StorageAdapter.deleteUser(user.id).then(finish).catch(finish);
            }
            finish();
          });
        });
    ...
    // ── Conquistas (só aparece depois da primeira) ──
    if (window.Achievements) {
      const owned = Achievements.unlocked(user);

      if (owned && owned.length) {
        const achCard = document.createElement('div');
        achCard.className = 'card';
        achCard.style.marginTop = '16px';
        achCard.style.background = 'rgba(16,15,12,0.85)';
        achCard.style.backdropFilter = 'blur(10px)';
        achCard.style.border = '1px solid var(--border)';

        const achTitle = document.createElement('div');
        achTitle.className = 'card-title';
        achTitle.textContent = '🏆 Conquistas (' + owned.length + ')';
        achCard.appendChild(achTitle);

        const achHint = document.createElement('div');
        achHint.style.fontSize = '12px';
        achHint.style.color = 'var(--ink-faint)';
        achHint.style.marginBottom = '12px';
        achHint.textContent = 'Escolha UMA conquista para exibir ao lado do seu nome.';
        achCard.appendChild(achHint);

        const achGrid = document.createElement('div');
        achGrid.style.display = 'grid';
        achGrid.style.gridTemplateColumns = 'repeat(auto-fill, minmax(210px, 1fr))';
        achGrid.style.gap = '8px';

        const displayedNow = Achievements.getDisplayed();

        function makeOption(id, label, icon, rarity, desc) {
          const rar = Achievements.rarityInfo(rarity);
          const btn = document.createElement('button');
          btn.className = 'btn-ghost';
          btn.style.display = 'flex';
          btn.style.alignItems = 'center';
          ...
          btn.style.borderRadius = 'var(--r)';
          btn.style.transition = 'all var(--t-fast)';
          const active = (displayedNow || '') === (id || '');
          btn.style.border = active ? '2px solid ' + rar.color : '1px solid var(--border)';
          btn.style.background = active ? 'rgba(212,184,75,.08)' : 'transparent';
          ...
          tx.style.color = id ? rar.color : 'var(--ink-faint)';
          tx.textContent = label;
          txtWrap.appendChild(tx);

          if (id) {
            const rr = document.createElement('span');
            rr.style.display = 'block';
            rr.style.fontSize = '10px';
            rr.style.letterSpacing = '.1em';
            rr.style.textTransform = 'uppercase';
            rr.style.color = 'var(--ink-faint)';
            rr.textContent = rar.label;
            txtWrap.appendChild(rr);
          }

          btn.appendChild(txtWrap);

          btn.addEventListener('click', function() {
            Achievements.setDisplayed(id || '');
            Dice.sounds.click();
            Toast.success(id ? 'Exibindo "' + label + '" no seu perfil.' : 'Nenhuma conquista em exibição.');
            rerender();
          });
        ...
        // Opção: não exibir nada
        achGrid.appendChild(makeOption('', 'Não exibir nenhuma', '🚫', 'comum', ''));

        owned.forEach(function(id) {
          const def = Achievements.get(id) || { id: id, icon: '❔', title: id, rarity: 'comum', desc: '' };
          achGrid.appendChild(makeOption(def.id, def.title, def.icon || '🏆', def.rarity, def.desc));
        });

        achCard.appendChild(achGrid);
        mainCard.appendChild(achCard);
      }
    }
  ...
  return { render: render, renderView: renderView };
})();