...
const tableId = 'mesa_' + PouchInit.generateId().slice(0, 8);
      ...
      StorageAdapter.save(table)
        .then(() => {
          ...
          Toast.success('Link do convite: ' + inviteLink, '🎲 Mesa criada', { duration: 9000 });
          resolve(table);
        })
        .catch(reject);
    });
    ...
    return new Promise((resolve, reject) => {
      ...
      StorageAdapter.get('table_' + tableId)
        ...
        .then(() => {
          ...
          Toast.success('Você entrou na mesa como jogador.', '🎲 Bem-vindo');
          resolve({ id: tableId });
        })
        .catch(reject);
    });
  ...
  function shareCharacter(char, tableId, options = {}) {
    return new Promise((resolve, reject) => {
      ...
      tableDb.put(doc)
        .then(() => {
          Toast.success('Personagem compartilhado na mesa.');
          resolve(doc);
        })
        .catch(reject);
    });
