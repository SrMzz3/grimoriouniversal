...
var doc = {
  ...
  // ================================================================
  // AUTH VIA BACKEND FLASK (/api/auth/*)
  // O GrimorioStorage (perfil/select) continua lendo de 'grimorio_users'
  // no localStorage, então toda resposta do backend é espelhada lá para
  // manter o resto do app funcionando sem outras mudanças.
  // ================================================================
...
function loadUserFromSession() {
  ...
  function registerUser(username, password) {
    return apiRequest('/register', 'POST', { username: username, password: password });
  }

  function loginUser(username, password) {
    return apiRequest('/login', 'POST', { username: username, password: password })
      .then(function(user) {
        setCurrentUser(user);
        return user;
      });
  ...
  function recoverAccount(username, keyword, newPassword) {
    return apiRequest('/recover', 'POST', {
      username: username,
      keyword: keyword,
  ...
  function updateUser(userData) {
    ...
    return apiRequest('/user/' + userData.id, 'PUT', userData)
      .then(function(merged) {
        currentUser = merged;
        syncUserToLocalStorage(merged);
        localStorage.setItem('grimorio_session', JSON.stringify({ id: merged.id, username: merged.username }));
        return merged;
      });
  }

  function deleteUser(userId) {
    return apiRequest('/user/' + userId, 'DELETE')
